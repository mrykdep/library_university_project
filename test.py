...
for i in range(1,100):
    isbn += i
    payload = {'isbn': str(isbn+i), 'member_id':ADMIN_ID}
    response = requests.post(ADRESS+'operator/borrow_book', data = json.dumps(payload), headers = headers, auth=HTTPBasicAuth(ADMIN_ID, ADMIN_PASSWORD))
